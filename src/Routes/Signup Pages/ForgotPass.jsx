...
export default function ForgotPass() {
  ...
  const [message, setMessage] = useState('');
  const [error, setError] = useState('');
  const [resetLink, setResetLink] = useState(''); // New state for the reset URL

  const handleRequestResetLink = async () => {
    try {
      const res = await axios.post("http://localhost:8000/api/v1/users/forgotPassword", { email });
    setMessage(res.data.msg);
      setResetLink(res.data.resetURL); // Set the reset URL in the state
      setError('');
    } catch (err) {
      setError(err.response?.data?.msg || 'Something went wrong');
      setMessage('');
      setResetLink(''); // Clear reset URL on error
    }
  };
  ...
  return (
    <div className={styles.loginPage}>
      ...
      <div className={styles.container}>
        ...
        {/* Display the reset link if it's generated */}
        {resetLink && (
          <p>
